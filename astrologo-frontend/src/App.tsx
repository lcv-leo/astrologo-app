    ...
    return (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-md animate-in fade-in duration-300">
            <div className={`bg-white/90 backdrop-blur-2xl border ${content.borderColor} p-6 md:p-8 rounded-3xl max-w-2xl w-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] relative overflow-y-auto max-h-[90vh]`}>
                <button onClick={onClose} aria-label="Fechar Modal" title="Fechar" className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition"><X className="w-5 h-5 text-slate-600" /></button>
                ...
                <button onClick={onClose} aria-label="Compreendido" title="Compreendido" className="mt-8 w-full py-4 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold uppercase tracking-wider transition shadow-lg">Compreendido</button>
            </div>
        </div>
    );
};
    ...
    return (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white/90 backdrop-blur-2xl border border-white p-6 md:p-8 rounded-3xl max-w-md w-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] relative">
                <button onClick={onClose} aria-label="Fechar E-mail" title="Fechar" disabled={isSending} className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition disabled:opacity-50"><X className="w-5 h-5 text-slate-600" /></button>
                ...
                <input type="email" id="emailConsulente" aria-label="Endereço de E-mail" title="Endereço de E-mail" placeholder="usuario@email.com" className="w-full p-4 bg-slate-50 text-slate-800 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition shadow-inner mb-6" value={email} onChange={e => setEmail(e.target.value)} disabled={isSending} />
    ...
    return (
        <div className={`mt-10 pt-10 border-t border-${colorTheme}-200 animate-in slide-in-from-top-4 duration-700 w-full`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <h2 className={`text-2xl md:text-3xl font-black flex items-center gap-3 ${colorHex}`}><Icon className="w-8 h-8 flex-shrink-0" /> <span className="leading-tight">{titulo}</span></h2>
                <button type="button" aria-label="Saiba mais" title="Saiba mais" onClick={onInfoClick} className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-full border transition-all text-xs font-bold uppercase tracking-wider shadow-sm bg-white hover:shadow-md border-${colorTheme}-200 ${colorHex} hover:bg-${colorTheme}-50`}><HelpCircle className="w-4 h-4" /> Saiba mais</button>
            </div>
    ...
    return (
        <div className="w-full animate-in fade-in duration-700 max-w-5xl mx-auto mt-8">
            ...
            <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-10">
                ...
                <button onClick={whatsapp} aria-label="WhatsApp" title="WhatsApp" className="flex-1 min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-4 py-3 rounded-full transition-all text-[11px] md:text-sm font-bold uppercase tracking-wider border border-emerald-200 shadow-sm hover:shadow-md"><Share2 className="w-4 h-4" /> WhatsApp</button>
                <button onClick={() => setEmailModalOpen(true)} aria-label="E-mail" title="E-mail" className="flex-1 min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 bg-blue-50 text-blue-700 hover:bg-blue-100 px-4 py-3 rounded-full transition-all text-[11px] md:text-sm font-bold uppercase tracking-wider border border-blue-200 shadow-sm hover:shadow-md"><Mail className="w-4 h-4" /> E-mail</button>
            </div>
