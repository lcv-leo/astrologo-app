...
import { Database, RefreshCw, Trash2, Star, Sun, Moon, Sparkles, Wind, Hash, X, CheckCircle2, AlertCircle } from 'lucide-react';

const ADMIN_VERSION = "2.08.00";
...
// Traduções dinâmicas para Retrocompatibilidade com mapas antigos
...
const GlassToast: React.FC<{ config: NotificationConfig; onClose: () => void }> = ({ config, onClose }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[999999] animate-in slide-in-from-bottom-8 fade-in duration-300">
      <div className={`bg-white/90 backdrop-blur-2xl border ${isErr ? 'border-red-200 text-red-600' : 'border-emerald-200 text-emerald-600'} p-4 px-6 rounded-2xl shadow-xl flex items-center gap-3 font-bold text-[10px] md:text-sm`}>
        {isErr ? <AlertCircle className="w-5 h-5 flex-shrink-0" /> : <CheckCircle2 className="w-5 h-5 flex-shrink-0" />}
        <span>{config.message}</span>
        <button onClick={onClose} className="ml-4 text-slate-400 hover:text-slate-600" title="Concluído" aria-label="Fechar notificação"><X className="w-4 h-4" /></button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white/95 backdrop-blur-2xl border border-blue-200 p-6 md:p-8 rounded-[2rem] max-w-sm w-full shadow-2xl relative text-center">
        ...
        <h2 className="text-base md:text-lg font-black text-slate-800 mb-2">Atenção Crítica</h2>
        <p className="text-slate-600 text-[10px] md:text-xs mb-8 leading-relaxed">Você está prestes a expurgar o registro de <strong>{config.nome}</strong>. Esta ação não poderá ser desfeita.</p>
        <div className="flex gap-3 w-full">
          <button onClick={onCancel} className="flex-1 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold uppercase tracking-wider text-[10px] md:text-xs transition">Cancelar</button>
          <button onClick={() => onConfirm(config.id)} className="flex-1 py-3.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold uppercase tracking-wider text-[10px] md:text-xs transition shadow-lg shadow-red-500/30">Apagar</button>
        </div>
      </div>
    </div>
  );
...
const RenderBlocoAstrologico: React.FC<BlocoProps> = ({ titulo, dadosAstrologia, dadosUmbanda, icon: Icon, isTropical }) => {
  ...
  return (
    <div className={`mt-8 pt-8 border-t border-${cT}-200 animate-in slide-in-from-top-4 w-full`}>
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h2 className={`text-base md:text-lg font-black flex items-center gap-2 ${cH}`}><Icon className="w-5 h-5 md:w-6 md:h-6 flex-shrink-0" /> <span className="leading-tight text-balance">{titulo}</span></h2>
      </div>
      <div className="bg-white/60 backdrop-blur-xl p-4 md:p-6 rounded-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] w-full mb-6">
        <h3 className="text-sm md:text-base font-bold text-slate-800 mb-4 border-b border-slate-200 pb-2">I. Astrologia ({isTropical ? '12 Signos' : '13 Signos'})</h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {dadosAstrologia.map((a, i) => <div key={i} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm"><p className="text-[9px] md:text-[10px] text-slate-500 mb-1 font-medium truncate">{a.astro}</p><p className="font-bold flex items-center gap-2 text-slate-800 text-xs sm:text-sm truncate">{a.simbolo} {a.signo}</p></div>)}
        </div>
      </div>
      <div className="bg-white/60 backdrop-blur-xl p-4 md:p-6 rounded-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] w-full">
        <h3 className={`text-sm md:text-base font-bold ${cH} mb-4 border-b border-slate-200 pb-2`}><Moon className="inline w-4 h-4 md:w-5 md:h-5" /> II. Umbanda</h3>
        <div className="grid grid-cols-3 gap-2 md:gap-4">
          {dadosUmbanda.map((u, i) => (
            <div key={i} className={`flex flex-col items-center justify-between p-2 md:p-4 bg-white rounded-xl border border-slate-100 shadow-sm w-full h-full hover:shadow-md transition`}>
              <span className="text-xl md:text-3xl mb-1 md:mb-2">{u.simbolo}</span>
              <div className="flex items-center justify-center w-full mb-1 md:mb-2 h-8"><p className="text-[7px] sm:text-[8px] md:text-[9px] text-slate-500 font-bold uppercase text-center leading-tight line-clamp-2 text-balance">{formatPosicaoLabel(u.posicao)}</p></div>
              <div className={`w-full mt-auto ${bgSoft} py-1.5 md:py-2 px-1 rounded-lg border border-${cT}-200`}><p className={`text-[8px] sm:text-[9px] md:text-xs font-black ${cH} uppercase tracking-widest text-center truncate`}>{u.orixa}</p></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="w-full animate-in fade-in duration-700 max-w-5xl mx-auto mt-6">
      ...
      <div className="w-full my-10 relative group max-w-4xl mx-auto animate-in zoom-in duration-1000">
        ...
        <div className="relative w-full bg-white/80 backdrop-blur-2xl border border-white/50 py-6 px-5 md:px-8 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center overflow-hidden">
    ...
    <div className="min-h-screen bg-transparent text-slate-800 font-sans flex flex-col items-center w-full overflow-x-hidden relative">
      ...
      <div className="max-w-6xl mx-auto w-full flex flex-col items-center flex-grow p-3 sm:p-5 md:p-6">
        <header className="text-center mb-8 md:mb-10 w-full flex flex-col items-center px-2 pt-4 animate-in fade-in slide-in-from-top-4">
          ...
          <h1 className="w-full text-center font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-600 mb-2 uppercase text-[clamp(10px,2vw,24px)] text-balance">Santuário Mestre</h1>
        </header>
        ...
        {loadingList ? (
          ...
          <div className="w-full flex flex-col items-center max-w-5xl animate-in fade-in">
            <div className="w-full lg:w-max min-w-[50%] max-w-full bg-white/80 backdrop-blur-2xl border border-white rounded-[2.5rem] overflow-hidden mb-8 shadow-[0_8px_32px_rgba(0,0,0,0.08)] flex flex-col mx-auto">
              <div className="bg-blue-50/80 p-4 px-6 text-xs md:text-sm font-bold text-blue-700 border-b border-blue-100 uppercase tracking-widest flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-2"><Database className="w-4 h-4" /> Arquivo Akáshico</div>
              </div>

              <ul className="max-h-[400px] overflow-y-auto overflow-x-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-slate-50 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full divide-y divide-slate-100">
                {lista.map(item => (
                  <li key={item.id} className="flex items-stretch hover:bg-blue-50/30 group transition-colors min-w-max">
                    <button onClick={() => carregarMapa(item.id)} aria-label={`Carregar mapa de ${item.nome}`} className="flex-grow text-left px-6 py-5 text-slate-700 font-medium text-xs md:text-sm flex justify-between items-center outline-none">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setConfirmDialog({ show: true, id: item.id, nome: item.nome }); }} aria-label="Apagar Registro" className="px-6 py-5 text-slate-400 hover:text-red-500 hover:bg-red-50 opacity-50 md:opacity-0 md:group-hover:opacity-100 transition-all outline-none flex-shrink-0"><Trash2 className="w-4 h-4 md:w-5 md:h-5" /></button>
                  </li>
                ))}
                {lista.length === 0 && <li className="p-8 text-center text-slate-400 text-xs md:text-sm font-medium">O vazio sideral... Nenhum registro.</li>}
              </ul>

              <div className="bg-slate-50/90 p-4 px-6 flex justify-between items-center border-t border-slate-100">
                ...
                <button onClick={recarregarManual} disabled={isRefreshing} aria-label="Atualizar Lista" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-blue-600 hover:bg-blue-50 border border-slate-200 transition-all text-[10px] md:text-xs font-bold uppercase tracking-wider disabled:opacity-50 shadow-sm hover:shadow-md"><RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin' : ''}`} /> Atualizar</button>
              </div>
            </div>

            {selectedMap && (
              <div className="w-full border-t border-slate-200 pt-8 mt-4 animate-in slide-in-from-bottom-6">
                <h2 className="text-center text-lg md:text-xl text-slate-800 font-bold mb-8 bg-white/60 backdrop-blur-xl p-4 px-6 md:p-5 md:px-8 rounded-[2rem] border border-white shadow-[0_8px_32px_rgba(0,0,0,0.06)] inline-flex items-center justify-center mx-auto gap-3 text-balance">
