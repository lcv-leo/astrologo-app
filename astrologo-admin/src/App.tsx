    ...
    return (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white/90 backdrop-blur-2xl border border-rose-200 p-6 md:p-8 rounded-3xl max-w-md w-full shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative">
                <button onClick={onClose} aria-label="Fechar E-mail" title="Fechar" disabled={isSending} className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition disabled:opacity-50"><X className="w-5 h-5 text-slate-500" /></button>
...
const RenderBlocoAstrologico = ({ titulo, dadosAstrologia, dadosUmbanda, icon: Icon, isTropical, onInfoClick }: any) => {
    const cT = isTropical ? 'orange' : 'indigo'; const cH = isTropical ? 'text-orange-600' : 'text-indigo-600';
    return (
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-4">
                    {dadosUmbanda.map((u: any, i: number) => (
                        <div key={i} className={`flex flex-col items-center justify-between p-3 md:p-5 bg-white rounded-2xl border border-slate-100 shadow-sm w-full h-full hover:shadow-md transition`}>
                            ...
                            <div className="flex items-center justify-center w-full mb-2 h-8"><p className="text-[9px] md:text-xs text-slate-500 font-bold uppercase text-center leading-tight line-clamp-2 text-balance">{u.posicao}</p></div>
                            <div className={`w-full mt-auto bg-${cT}-50 py-2 px-1 rounded-xl border border-${cT}-200`}><p className={`text-[10px] md:text-sm font-black ${cH} uppercase tracking-widest text-center truncate`}>{u.orixa}</p></div>
                        </div>
                    ))}
                </div>
                {!isTropical && (
    ...
    return (
        <div className="w-full animate-in fade-in duration-700 max-w-5xl mx-auto mt-8">
            ...
            <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-10">
                ...
                <button onClick={() => setEmailModalOpen(true)} aria-label="Enviar por E-mail" title="E-mail" className="flex-1 min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 bg-rose-50 text-rose-700 hover:bg-rose-100 px-4 py-3 rounded-full transition-all text-[11px] md:text-sm font-bold uppercase tracking-wider border border-rose-200 shadow-sm hover:shadow-md"><Mail className="w-4 h-4" /> E-mail</button>
            </div>
            ...
            <div className="w-full my-12 relative group max-w-5xl mx-auto animate-in zoom-in duration-1000">
                ...
                <div className="relative w-full bg-white/80 backdrop-blur-2xl border border-white/50 py-8 px-6 md:px-10 rounded-[2.5rem] shadow-xl shadow-indigo-100/50 flex flex-col items-center justify-center text-center overflow-hidden">
    ...
    const carregarMapa = async (id: string) => {
        try {
            ...
            if (data.success) {
                const m = data.mapa;
                setSelectedMap({
                    id: m.id, query: { nome: m.nome, data_nascimento: m.data_nascimento, horaNascimento: m.hora_nascimento, localNascimento: m.local_nascimento },
            ...
            <div className="max-w-6xl mx-auto w-full flex flex-col items-center flex-grow p-3 sm:p-6 md:p-8">
                ...
                {loadingList ? (
                    ...
                    <div className="w-full flex flex-col items-center max-w-5xl animate-in fade-in">
                        <div className="w-full max-w-md bg-white/80 backdrop-blur-2xl border border-white rounded-[2.5rem] overflow-hidden mb-8 shadow-[0_8px_32px_rgba(0,0,0,0.06)] flex flex-col">
