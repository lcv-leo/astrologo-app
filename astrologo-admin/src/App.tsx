  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300">
      <div className={`bg-white/90 backdrop-blur-2xl border ${content.borderColor} p-6 md:p-8 rounded-3xl max-w-2xl w-full shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative`}>
        <button onClick={onClose} aria-label="Fechar Informações" className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition"><X className="w-5 h-5 text-slate-500" /></button>
        ...
        <button onClick={onClose} className="mt-8 w-full py-4 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-xl font-bold uppercase tracking-wider transition border border-slate-200">Compreendido</button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white/90 backdrop-blur-2xl border border-rose-200 p-6 md:p-8 rounded-3xl max-w-md w-full shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative">
        <button onClick={onClose} disabled={isSending} aria-label="Fechar Envio de E-mail" className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition disabled:opacity-50"><X className="w-5 h-5 text-slate-500" /></button>
        ...
        <input type="email" id="emailAdmin" placeholder="usuario@email.com" className="w-full p-4 bg-white text-slate-800 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none mb-6 shadow-sm" value={email} onChange={e => setEmail(e.target.value)} disabled={isSending} />
  ...
  return (
    <div className={`mt-10 pt-10 border-t border-${cT}-200 animate-in slide-in-from-top-4 w-full`}>
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <button onClick={onInfoClick} className={`flex items-center gap-2 px-5 py-2.5 rounded-full border text-xs font-bold uppercase shadow-sm bg-white border-${cT}-200 ${cH} hover:bg-${cT}-50`}><HelpCircle className="w-4 h-4" /> Saiba mais</button>
      </div>
      <div className="bg-white/60 backdrop-blur-xl p-5 md:p-8 rounded-[2rem] border border-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] w-full mb-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {dadosAstrologia.map((a: any, i: number) => <div key={i} className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm"><p className="text-[10px] text-slate-500 mb-1 font-medium">{a.astro}</p><p className="font-bold flex items-center gap-2 text-slate-800 text-xs sm:text-sm">{a.simbolo} {a.signo}</p></div>)}
        </div>
      </div>
...
const ResultView = ({ result, analiseIa, openInfoModal }: any) => {
  ...
  const dispararEmail = async (emailDestino: string) => {
    ...
    try {
      // Ponte Cross-Origin para a API de envio de E-mail residente no Frontend
  ...
  return (
    <div className="w-full animate-in fade-in duration-700 max-w-5xl mx-auto mt-8">
      ...
      <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-10">
        ...
        <button onClick={whatsapp} aria-label="WhatsApp" className="flex-1 min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-4 py-3 rounded-full transition-all text-[11px] md:text-sm font-bold uppercase tracking-wider border border-emerald-200 shadow-sm hover:shadow-md"><Share2 className="w-4 h-4" /> WhatsApp</button>
        <button onClick={() => setEmailModalOpen(true)} aria-label="E-mail" className="flex-1 min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 bg-rose-50 text-rose-700 hover:bg-rose-100 px-4 py-3 rounded-full transition-all text-[11px] md:text-sm font-bold uppercase tracking-wider border border-rose-200 shadow-sm hover:shadow-md"><Mail className="w-4 h-4" /> E-mail</button>
      </div>
...
export default function App() {
  ...
  const buscarRegistros = async (isUpdate = false) => {
    if (isUpdate) setIsRefreshing(true); else setLoadingList(true);
    try {
      const res = await fetch(`/api/admin/listar`);
      const data = await res.json();
      if (data.success) { setLista(data.mapas); } else { console.error(data.error); }
    } catch (e) { console.error("Falha na comunicação com o banco."); }
  ...
  const carregarMapa = async (id: string) => {
    ...
    } catch (err) { alert("Falha ao abrir os registros."); }
  };
    ...
    <div className="min-h-screen bg-transparent text-slate-800 font-sans flex flex-col items-center w-full overflow-x-hidden relative">
      ...
      <div className="max-w-6xl mx-auto w-full flex flex-col items-center flex-grow p-3 sm:p-6 md:p-8">
        <header className="text-center mb-10 md:mb-14 w-full flex flex-col items-center px-2 pt-4 animate-in fade-in slide-in-from-top-4">
          <div className="p-4 bg-white/60 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white mb-6"><Database className="w-12 h-12 md:w-16 md:h-16 text-rose-600" /></div>
              ...
              <ul className="max-h-[300px] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-slate-50 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full divide-y divide-slate-100">
                {lista.map(item => (
                  <li key={item.id} className="flex items-stretch hover:bg-rose-50/50 group transition-colors">
                    <button onClick={() => carregarMapa(item.id)} aria-label={`Carregar mapa de ${item.nome}`} className="flex-grow text-left px-6 py-5 text-slate-700 font-medium text-sm flex justify-between items-center overflow-hidden outline-none">
                    ...
                    <button onClick={(e) => deletarMapa(item.id, item.nome, e)} aria-label="Apagar Registro" className="px-6 py-5 text-slate-400 hover:text-red-500 hover:bg-red-50 opacity-50 md:opacity-0 md:group-hover:opacity-100 transition-all outline-none flex-shrink-0"><Trash2 className="w-5 h-5" /></button>
                  </li>
                ))}
